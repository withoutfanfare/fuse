...
use crate::github;
use crate::models::PrCommentsResponse;

use super::CommandError;

/// Fetch comments and reviews for a pull request from GitHub.
///
/// Looks up the repository full name and PR number from the database,
/// then queries the GitHub CLI for comments and review threads.
/// Uses async subprocess execution to avoid blocking the thread pool.
#[tauri::command]
pub async fn fetch_pr_comments(
    pr_id: i64,
    state: State<'_, DbState>,
) -> Result<PrCommentsResponse, CommandError> {
    ...
    let json_value = github::fetch_comments_async(&full_name, number).await?;
    let response: PrCommentsResponse = serde_json::from_value(json_value)?;
    Ok(response)
}
