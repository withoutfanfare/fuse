...
**Fuse** is a Tauri v2 desktop application for PR monitoring, triage, and AI-assisted code review. It syncs pull requests via the GitHub CLI (`gh`), scores them by risk, and provides AI review assistance through Codex CLI. macOS is the primary target platform.
...
- **External CLIs**: `gh` (GitHub), `grove` (worktrees, optional), `Codex` (AI reviews, optional) — all invoked via `tauri-plugin-shell`.
